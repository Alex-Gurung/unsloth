...
if HAS_CUDA_STREAM:
    ...
    def fast_dequantize(W, quant_state = None, out = None, use_global_buffer = False):
        ...
        if use_global_buffer:
            ...
            if WEIGHT_BUFFER is None:
                WEIGHT_BUFFER = torch.empty(size, dtype = dtype, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"], requires_grad = False)
                ABSMAX_BUFFER = torch.empty(n_elements_absmax, dtype = torch.float32, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"], requires_grad = False)
    ...
    def fast_dequantize(W, quant_state = None, out = None, use_global_buffer = False):
        ...
        if use_global_buffer:
            ...
            if WEIGHT_BUFFER is None:
                WEIGHT_BUFFER = torch.empty(size, dtype = dtype, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"], requires_grad = False)
                ABSMAX_BUFFER = torch.empty(n_elements_absmax, dtype = dtype, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"], requires_grad = False)
