...
class Fast_CrossEntropyLoss(torch.autograd.Function):
    ...
    def forward(ctx, logits, labels, logit_softcapping : float = 0, logit_scaling : float = 0):
        ...
        losses = torch.empty(n_rows, dtype = torch.float32, device = torch.cuda.current_device())
        ...
        if n_chunks == 1:
            ...
            logsumexp = torch.empty(n_rows, dtype = torch.float32, device = torch.cuda.current_device())
            ...
            logsumexp = torch.empty((n_rows, n_chunks,), dtype = torch.float32, device = torch.cuda.current_device())
