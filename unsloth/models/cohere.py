...
def CohereDecoderLayer_fast_forward(
    ...
    *args, **kwargs,
    ...
    if use_cache and hasattr(self, "_flag_for_generation"): #past_key_value is not None:
        out_weight = torch.empty(self.input_layernorm.weight.shape, dtype = torch.float32, device = torch.cuda.current_device())
...
def CohereAttention_fast_forward_inference(
    ...
    if do_prefill:
        ...
        if self.use_qk_norm:
            self.q_norm_out_weight = torch.empty(self.q_norm.weight.shape, dtype = torch.float32, device = torch.cuda.current_device())
            self.k_norm_out_weight = torch.empty(self.k_norm.weight.shape, dtype = torch.float32, device = torch.cuda.current_device())
        else:
...
def CohereModel_fast_forward_inference(
    ...
    out_weight = torch.empty_like(self.model.layers[0].input_layernorm.weight, dtype = torch.float32, device = torch.cuda.current_device())
