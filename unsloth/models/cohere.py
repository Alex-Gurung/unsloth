...
def CohereAttention_fast_forward_inference(
    ...
    if do_prefill:
        ...
        if attention_size != hidden_size:
            self.temp_O = torch.empty((1, bsz, hidden_size), dtype = dtype, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"])
        else:
        ...
        if self.use_qk_norm:
            self.q_norm_out_weight = torch.empty(self.q_norm.weight.shape, dtype = torch.float32, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"])
            self.k_norm_out_weight = torch.empty(self.k_norm.weight.shape, dtype = torch.float32, device = os.environ["UNSLOTH_PROCESS_CUDA_DEVICE"])
        else:
