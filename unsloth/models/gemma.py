...
class GemmaFixedRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        cos = emb.cos().to(device = torch.cuda.current_device(), non_blocking = True)#, dtype = dtype)
        sin = emb.sin().to(device = torch.cuda.current_device(), non_blocking = True)#, dtype = dtype)
