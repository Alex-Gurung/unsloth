...
def LlamaAttention_fast_forward_inference(
    ...
    if do_prefill:
        self.paged_attention = torch.empty((KV_CACHE_INCREMENT+seq_len+1, 2, bsz, n_kv_heads, head_dim), dtype = dtype, device = torch.cuda.current_device())
        self.paged_attention_K = self.paged_attention[:,0]
        self.paged_attention_V = self.paged_attention[:,1]
        self.paged_attention_K[:seq_len] = K1.permute(2, 0, 1, 3)
        self.paged_attention_V[:seq_len] = V1.permute(2, 0, 1, 3)
        self.temp_QA = torch.empty((2, bsz, 1, attention_size), dtype = dtype, device = torch.cuda.current_device())
        self.temp_KV = torch.empty((2, bsz, 1, n_kv_heads*head_dim), dtype = dtype, device = torch.cuda.current_device())
        self.RH_Q = torch.empty((bsz, n_heads, 1, head_dim), dtype = dtype, device = torch.cuda.current_device())
        
        # Mistral Nemo 12b has weird dimensions
        if attention_size != hidden_size:
            self.temp_O = torch.empty((1, bsz, hidden_size), dtype = dtype, device = torch.cuda.current_device())
        else:
            self.temp_O = self.temp_QA[1][:,:,:hidden_size]
        pass
        
        self.attention = torch.empty((bsz, n_heads, 1, KV_CACHE_INCREMENT+seq_len), dtype = dtype, device = torch.cuda.current_device())
    ...
    RH_K = RH_Q[:,:n_kv_heads,:,:] # torch.empty((n_kv_heads, 1, head_dim), dtype = dtype, device = torch.cuda.current_device())
    RH_K[:,:,:,:h] = Kn[:,:,:,h:]
    RH_K[:,:,:,h:] = Kn[:,:,:,:h]
...
def fast_swiglu_inference(self, X, temp_gate = None, temp_up = None):
    ...
    # temp = torch.empty((2, bsz, 1, mlp_size), dtype = X.dtype, device = torch.cuda.current_device())
        ...
        if HAS_FLASH_ATTENTION_SOFTCAPPING and attention_mask is None:
            ...
            if HAS_FLEX_ATTENTION:
                ...
                self.SWA_mask = AttentionMaskConverter(
                    ...
                    .to_causal_4d(1, n, n, dtype = inputs_embeds.dtype, device = torch.cuda.current_device(),)\
                    .squeeze(0).squeeze(0)
...
def LlamaModel_fast_forward_inference(
    ...
    residual = torch.empty((bsz, q_len, hd), dtype = torch.float32, device = torch.cuda.current_device())
    _XX = torch.empty((2, bsz, q_len, hd), dtype = torch.float32, device = torch.cuda.current_device())
    XX, XX2 = _XX[0], _XX[1]
    variance = torch.empty((bsz, q_len, 1), dtype = torch.float32, device = torch.cuda.current_device())
    temp_mlp = torch.empty((2, bsz, 1, mlp_size), dtype = X.dtype, device = torch.cuda.current_device())
        ...
        *args, **kwargs,
        ...
        if labels is not None:
            ...
            if not hasattr(self, "extra_ignored_labels"):
                ...
                self.extra_ignored_labels = torch.full((self.max_seq_length, 1), -100, device = torch.cuda.current_device())
            pass
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def extend_rope_embedding(self, x, seq_len):
        ...
        self._set_cos_sin_cache(self.current_rope_size, device = torch.cuda.current_device(), dtype = x.dtype)
    pass
pass
...
class FastLlamaModel:
            ...
            if check_all:
                ...
                if "embed_tokens" in new_target_modules:
                    ...
                    model.get_input_embeddings().modules_to_save.default\
                        .to(device = torch.cuda.current_device(), dtype = new_dtype, non_blocking = True)
    ...
    def patch_peft_model(
        ...
        extra_ignored_labels = torch.full((max_seq_length, 1), -100, device = torch.cuda.current_device())
